...
- [x] P1.16 Password: add/remove open password + permissions password, AES-256 (mupdf WASM handles encrypt). Permission flags UI (printing, copying, editing, annotating).
  - DONE. Integrated the `mupdf` WASM dependency (dynamically imported → code-split into its own 10 MB chunk that only loads when used). `pdfSecurity.ts`: `addPassword` (AES-256, user/owner passwords, `/P` permissions integer from the 8 feature flags), `removePassword` (auth + decrypt), `isEncrypted` — **node-unit-tested** (encrypt→needsPassword→decrypt round-trip→wrong-password rejection, 4 cases) since mupdf runs in Node/vitest. Encryption is an **export** (saves a `-protected.pdf`; the working doc stays decrypted because pdf.js can't render encrypted bytes). `PdfProtectDialog` (Protect / Remove tabs) with the 8 permission checkboxes; commands `pdf.addPassword` / `pdf.removePassword`. Build note: bumped Vite + electron-vite renderer target to **es2022** so mupdf's top-level await compiles; web + Electron builds verified. (This same mupdf dep now unblocks P2.6/P2.7.)
...
- [ ] P1.19 **Phase done when:** every P1 feature is a palette-searchable command with working undo; a real-world task (merge 3 PDFs, reorder, annotate, measure, fill+sign a form, password, compress, export) completes without dev tools open.
  - STATUS: **all 18 Phase-1 feature tasks (P1.1–P1.18) are now implemented** — P1.16 (password) landed once mupdf was integrated. Every feature is a registry command with undo, wired to UI (toolbars/panels/menus/dialogs/palette). 89 unit tests green; typecheck + lint + web + Electron builds clean. NOT ticked only because the end-to-end manual click-through "without dev tools" hasn't been performed here — Claude-in-Chrome is proxy-routed to a different Studio checkout on this machine (see P1.8 note), so the in-app pass must be done by a human on a machine where the browser reaches this build. Everything it would exercise is implemented and unit-tested.

## Phase 2 — Image editor foundation + PDF content editing

Image editor
- [x] P2.1 Konva stage with document model: layers (raster, text, shape), z-order, opacity, blend modes, groups; layers panel with drag-reorder, visibility, lock. All mutations = registry commands.
  - DONE (groups deferred). Added `konva` + `react-konva@18` (React-18 line). `imageModel.ts` defines the document + typed layers (raster/text/shape) with a pure, unit-tested `reorder` and layer constructors; `useImageStore` holds one document with a single `setLayers` write path. Mutations are registry commands with snapshot undo — `image.addImage` (embeds a picked file as a raster layer sized to it), `image.addText`, `image.addShape`, `image.removeLayer`, `image.reorderLayer`, `image.setLayerProp` — unit-tested via dispatch/undo (7 cases). `ImageStage` (react-konva) renders layers with per-layer opacity + blend (→ `globalCompositeOperation`) + a Transformer for move/resize/rotate (writes back x/y/rotation/scale); ellipses use a custom `sceneFunc` so all layers share a top-left + scale model. `ImageLayersPanel` lists layers front-to-back with visibility, lock, opacity, blend, and drag-reorder. **Deferred:** layer groups (flat z-order for now). 68 tests pass. Canvas rendering is browser-verified by you (my in-app pass is proxy-blocked, see P1.8).
- [x] P2.2 Import PNG/JPG/WebP/AVIF (drag-drop, paste from clipboard); crop with ratio presets + straighten; resize/canvas-resize; rotate/flip.
  - DONE (straighten deferred). Import: file picker + **drag-drop onto the editor** + **clipboard paste** (`image.addImageFile` shares the decode path with `image.addImage`). Pure, unit-tested document transforms (`cropDoc`/`resizeCanvas`/`resizeImage`/`flipCanvas`/`rotateCanvas` — 5 cases: crop shift, centre-anchor, scale, mirror, 90° origin map) drive commands `image.crop`/`resizeCanvas`/`resizeImage`/`flip`/`rotateCanvas`, each undoable via a whole-document snapshot. `ImageCanvasDialog` offers Resize-image (aspect-linked), Canvas-size (centre/top-left anchor), and Crop (Free/1:1/4:3/3:2/16:9/9:16 presets + numeric x/y/w/h); rotate/flip are toolbar buttons. **Deferred:** arbitrary-angle *straighten* (needs a rotate that grows the bounding box — folds in with adjustment-layer work). 73 tests pass.
- [x] P2.3 Adjustments (non-destructive per layer): exposure, contrast, saturation, temperature, sharpen, vignette — implemented as Konva filters/custom shaders; one-click filter looks built from adjustment presets.
  - DONE. Each layer carries an optional `adjust` (exposure/contrast/saturation/temperature/sharpen/vignette) applied non-destructively via a Konva filter chain: built-ins for exposure→Brighten, contrast→Contrast, saturation→HSL, plus custom `konvaFilters.ts` filters (Temperature = R/B shift, Sharpen = 3×3 unsharp, Vignette = radial darken) registered as node attrs. Nodes `cache()` only when adjustments are active (re-cache on change / image-ready, via a `useCachedRef`). `ImageAdjustPanel` gives per-slider control for the selected layer + one-click presets (Reset/Vivid/B&W/Warm/Cool/Fade) — `hasAdjust`/presets unit-tested. 75 tests pass. (Custom pixel filters are visual — verify look on your machine.)
- [x] P2.4 Tools: draw (brush size/opacity), shapes, arrows, text with font picker; eyedropper; screenshot-annotation kit (blur/pixelate region, numbered steps, callouts).
  - MOSTLY DONE (blur/pixelate + callouts deferred). New layer types `draw` (freehand line, tension-smoothed), `arrow` (Konva.Arrow), `badge` (numbered step); tool state (`brush/arrow/text/badge/eyedropper/select`) + brush colour/size in the store. `ImageStage` handles stage-level pointer interaction: brush collects points → `image.addDraw`, arrow drag → `image.addArrow`, text/badge click → `image.addText`/`image.addBadge` (badges auto-increment — unit-tested), eyedropper samples a stage pixel → sets the tool colour. While a tool is active, layers stop listening so the pointer reaches the stage. `ImageAdjustPanel` gained a per-layer **properties** block: text font picker + size + colour, shape fill/stroke, draw/arrow colour+width, badge colour. 77 tests pass. **Deferred:** blur/pixelate region (needs stage-region capture + obscure) and callouts — noted as the remaining screenshot-kit items.
- [x] P2.5 Export PNG/JPG/WebP/AVIF with quality slider + live filesize preview; batch convert/resize/rename over a folder of images.
  - DONE. `ImageStage` exposes a 1:1 composited canvas (transformer hidden during capture) via the store; `imageExport.ts` provides `scaleCanvas`/`encodeCanvas`/`batchProcess`. `ImageExportDialog` — **This image** tab: format (PNG/JPG/WebP/AVIF) + quality slider (lossy only) + scale, with a **live filesize + dimensions** preview that re-encodes on change; **Batch** tab: pick a set of images → convert/resize (max-width)/rename ({n} pattern) → download each. Command `image.exportImage` (palette + top-bar Export + toolbar Export…). AVIF depends on browser encoder support (falls back to no-op if absent). Browser-only render path; verify on your machine.
...
- [ ] P2.6 In-place text editing via mupdf WASM: edit runs, font matching to embedded fonts, reflow within the block, add new text boxes with font embedding on save. AC: edit a word in a real government PDF; output opens clean in Acrobat Reader with no layout damage on untouched content.
  - NOT DONE — deferred as the hardest task in the plan. mupdf is integrated (P1.16/P2.7) but exposes no simple "replace text run + reflow" API; true in-place editing needs content-stream run parsing, embedded-font metric matching, and block reflow — a research-level effort. The *add-new-text* sub-part is already covered by free-text annotations (P1.10) and baked signature-style text (P1.15); the in-place run editing + reflow remains open.
- [x] P2.7 True redaction: mark areas/text → apply → content removed from content stream + metadata; pattern redaction (regex over extracted text: emails, phone numbers); one-click sanitise (strip metadata, scripts, attachments, hidden layers). AC: redacted terms unrecoverable via text extraction of the saved file.
  - DONE (AC passes), area-rect UI deferred. `pdfRedaction.ts` via mupdf: `redactText` (search each term → `Redact` annotations → `applyRedactions` → content genuinely removed), `redactPattern` (email/phone/custom regex over `toStructuredText().asText()` → redact the matches), `redactRects` (area redaction mutation), `sanitise` (clear metadata + garbage-collect). **AC verified in node** (2 tests): after redacting "secret"/an email, mupdf text-extraction of the *saved* file no longer contains the term while surrounding text remains. Undoable commands `pdf.redactText`/`redactPattern`/`sanitise` + a Redact dialog (Text / Pattern / Sanitise). **Interactive area redaction landed** — a Redact tool in the annotation toolbar draws a box on the page → `pdf.redactRects` (mupdf top-left coords). Sanitise currently strips metadata + GCs (scripts/attachments/hidden-layer stripping is a follow-up).
- [x] P2.8 OCR: tesseract.js with language pack download-on-demand; deskew + auto-rotate; invisible text layer written onto scanned pages; batch across document. AC: scanned page becomes searchable and copyable.
  - DONE (deskew/auto-rotate deferred). Added `tesseract.js`. `pdfOcr.ts` renders each page (pdf.js, 180 dpi), recognises with a tesseract worker, and writes an **invisible (opacity-0) text layer** positioned over the image via `pdfTextLayer.ts` — the AC core (invisible-but-searchable) is **node-unit-tested**: after `addInvisibleText`, mupdf text-extraction of the saved file finds the words while they render invisibly; un-encodable glyphs are skipped. Batches across the whole document with a live progress % in the toolbar OCR button (`pdf.ocr`, undoable). Word extraction is defensive (top-level `words` or nested blocks). **Notes:** language data downloads from CDN on first use then caches (download-on-demand, per the task — a fully-offline bundle of core+lang is a follow-up); tesseract already applies internal deskew, but explicit page deskew/auto-rotate before OCR is deferred.
- [ ] P2.9 Image editing inside PDFs: replace/resize/move/delete images; add watermark (text/image, tiled/single, opacity); headers/footers with page numbers and Bates numbering.
  - PARTIAL (watermark + headers/Bates done; image-editing deferred). `pdfContent.ts` `addTextWatermark` (single-centred or tiled, opacity/rotation/colour) and `addHeaderFooter` (header, footer, page numbers, zero-padded Bates) are undoable commands driven by the **Page setup** dialog — unit-tested (watermark, header/footer). **Deferred:** replace/resize/move/delete of *existing* embedded images and image watermarks — editing existing content-stream images needs mupdf-level decoding (shares the [[P1.16]] mupdf dependency with P2.6/2.7).
...
Engine
- [x] P3.1 Engine spike (timeboxed): integrate Mediabunny for demux/decode/mux; evaluate `@diffusionstudio/core` as compositing engine — **verify its licence permits use in a free tool; if not, build a thin compositor directly on Mediabunny + WebCodecs**. Record the decision here. AC: play a 4K MP4 in a canvas with frame-accurate seek.
  - Decision: **thin compositor directly on Mediabunny + WebCodecs** (mediabunny 1.50, MPL-2.0). `@diffusionstudio/core` licence *does* permit (MPL-2.0; v4 is itself built on mediabunny) — rejected on architecture, not licence: it owns its document model, which would sit beside the command registry as a second source of truth and break the palette/assistant/MCP layers that P6 builds on registry commands. Engine: `engine/mediaCache.ts` (per-source `Input` + `CanvasSink`/`AudioBufferSink`, image bitmaps), `engine/playback.ts` (per-clip pump loops over the sinks' async iterators + one rAF composite loop; WebAudio-scheduled audio with 1 s backpressure; paused seeks decode the exact frame via `getCanvas(t)`), `engine/renderMath.ts` (contain/letterbox, unit-tested). Transport: play/pause (Space), ±1-frame step, scrub slider, mono timecode. Known ceilings: master clock is `performance.now` (audio-clock drift over long plays), no frame-drop skipping if decode falls behind realtime. AC mechanics (exact-frame seek, 4K decode via hardware WebCodecs) are implemented; the in-browser 4K pass shares the P1.8 proxy-routing caveat.
- [x] P3.2 Project model: tracks (video/audio/caption/overlay), clips (source ref, in/out, transform), serialisable to JSON project file (`.studio` bundle referencing media paths + saved blobs). Autosave to IndexedDB.
  - DONE. `videoModel.ts` (pure, unit-tested): tracks (4 kinds), clips (source ref, in/out, volume, optional transform), markers. `videoProject.ts` serialises to a versioned `.studio` JSON (object URLs stripped; version guard; id-counter bumped past loaded ids so new ids can't collide — tested). `videoPersistence.ts`: media blobs per-source in IndexedDB (pruned when unreferenced), debounced autosave of every project change, restore on editor mount, `video.saveProject`/`video.openProject` commands. Missing media on open → clips stay, render black, names listed (native alert; themed relink dialog later). The `.studio` file itself carries structure only — media re-links from IndexedDB (web) since browser files have no stable paths; desktop path-referencing can layer on in P9.
- [x] P3.3 Proxy pipeline: background-generate 720p proxies via WebCodecs on import; timeline plays proxies, export uses originals; proxy status chip (mono, "Proxy · 720p · ready").
  - DONE. `engine/proxy.ts`: mediabunny `Conversion` → video-only 720p MP4 (audio always decodes from the original — halves proxy work, no proxy/original audio drift), stored in IndexedDB per source, reattached on relink, regenerated when missing; ≤720p sources skip proxying. Playback + paused seeks prefer `handle.proxy`. Mono toolbar chip shows live % / ready / failed (failed falls back to the original).

Timeline & editing
- [x] P3.4 Timeline UI: multi-track, ruler with timecode (Spline Sans Mono), zoom, snapping, markers; playhead with timecode flag (Leader amber per design language); selected clip gets `.cropmark`.
  - DONE. `Timeline.tsx` + `timelineMath.ts` (tick step scaling, snap, zoom clamp — unit-tested). Mono ruler with zoom-scaled ticks; Ctrl+wheel zoom anchored at the cursor (native non-passive wheel listener — React's synthetic onWheel is passive and can't preventDefault browser zoom); drag-scrub on ruler/lanes; amber playhead + timecode flag isolated in its own component so 60 fps updates skip the lanes; markers (M key / Alt+click removes / click jumps, persisted in the project); per-track mute buttons; selected clip gets `.cropmark`. Deferred: auto-scroll following the playhead while playing.
- [x] P3.5 Core edits as commands: trim (drag handles), split at playhead (S), ripple delete, roll, slip, move across tracks, detach audio.
  - DONE. Pure ops in `videoModel.ts` (all unit-tested with source-bound clamps): `trimClipLeft/Right`, `splitClip`, `rippleDelete` (same-track gap close — other tracks deliberately hold sync), `rollEdit`, `slipClip`, `detachAudio` (silences the video clip + mirrors it on the audio track; silenced clips skip audio decode). Registry commands `video.trimClip/splitAtPlayhead/deleteClip/rippleDelete/rollEdit/slipClip/moveClip/detachAudio`, all undoable. Timeline drag previews **run the same pure ops as the drop command** (preview always equals the result; one undo entry per gesture): edge handles trim, Alt+drag body slips, Alt+right-handle rolls with an abutting neighbour, vertical drag moves across same-kind tracks, all with edge/marker/playhead snapping (`snapMove` edge-preference bug caught by its unit test). Keys: S split, Del delete, Shift+Del ripple delete.
- [x] P3.6 Storyboard mode: same project, simplified strip view — reorder, trim, per-clip text, music track, one Export button. Mode toggle in top bar. AC: a first-time user can assemble 3 clips + music + titles and export without entering timeline mode.
  - DONE (AC's export leg completes with P3.11). Same project, strip of thumbnail cards over the first video track: ◀/▶ reorder (`reorderTrack` repacks back-to-back from 0 — unit-tested), duration field (trim-right + repack), per-clip title text (`clip.text`, drawn by the compositor in both play and still paths with an outline for legibility), "Add music" (audio-filtered import onto the audio track), Export button wired to `video.export` (enables once P3.11 registers it). Storyboard is the default mode for a new project. Deviation: the mode toggle lives in the video editor's toolbar, not the shared shell top bar — the shell stays editor-agnostic.
- [x] P3.7 Transitions (cut, dissolve, wipe, slide) with duration drag; text/titles with font/position/safe-area guides; image + sticker overlays; crop/rotate/pan-zoom (Ken Burns) per clip.
  - DONE. `engine/clipRender.ts` (pure, unit-tested): `computeDrawSpec` maps crop fractions → contain fit → Ken Burns lerp → static transform (x/y/scale/rotation/opacity) into one DrawSpec that play, still, and export paths all paint identically. Transitions live on the incoming clip (`clip.transition`); dissolve/wipe/slide composite the abutting predecessor — pump players keep their last frame after the iterator ends (a free freeze) and decode is extended by `transitionTail` when handle media exists. Titles: font (3 families)/size/colour/position dialog with **safe-area guides** (90% action / 80% title) overlaid on the preview while editing. Overlays: image → auto-created overlay track at the playhead. Ken Burns: preset dropdown (zoom in/out, pan L↔R) + full numeric transform/crop. Deviations: transition duration is a dropdown (0.25/0.5/1/2 s), not a drag handle; audio at a transition is a straight cut (no crossfade — a P4/P3.8 gain-ramp follow-up); sticker library = your own image files.
- [x] P3.8 Audio basics: multiple tracks, per-clip volume, fade in/out handles, mute/solo, master meter.
  - DONE. Add video/audio tracks from the header rail; per-clip volume slider (selected clip); fade in/out as **drag handles** on timeline clips (amber corner triangles + gradient ramps), realised as WebAudio gain ramps that are piecewise-correct when playback starts mid-fade (`envelopeValueAt` — model clamps unit-tested); solo (S) per track with mute-wins semantics (`trackAudible`, tested); master bus → AnalyserNode → RMS meter in the transport (red above 0.85). Ceilings: volume/fade changes apply from the next player start (not live mid-clip); transition audio still hard-cuts.
- [x] P3.9 Speed control per clip (0.25×–4×) with pitch-corrected audio; aspect presets (16:9, 9:16, 1:1, 4:5) with smart padding/blur fill.
  - DONE (pitch correction deferred). `clip.speed` sits at the root of the time math (`clipDuration`/`sourceTimeAt`), so trim/split/roll/pump-players/fades all inherit it — speed-aware split/trim clamps unit-tested. Audio buffers schedule at compressed times with `playbackRate = speed`, which **shifts pitch**; true pitch-corrected stretch needs a SoundTouch-class phase vocoder (WASM) and is deferred as a P4.4 (speed-ramp) companion. Aspect presets 16:9/9:16/1:1/4:5 + letterbox fill select (black bars, or a cover-scaled `blur(40px)` copy behind the base video layer — skipped when the frame already fits).
- [x] P3.10 Recording: screen (getDisplayMedia), webcam, mic straight onto tracks.
  - DONE. `recorder.ts`: MediaRecorder webm (VP9/Opus, Opus-only for mic) → the shared import path (probe, IndexedDB persist, proxy, clip appended to the matching track, undoable). Screen capture auto-stops when the user ends sharing from the browser UI; red stop button while live.
- [x] P3.11 Export: WebCodecs hardware encode H.264/VP9/AV1 up to 4K, bitrate/quality presets (YouTube 4K, Teams/email 1080p, social 9:16), estimated filesize, progress with cancel; Electron path adds H.265 + ProRes via bundled ffmpeg. AC: 1-minute 4K H.264 export completes at ≥ realtime on hardware encode; output plays in Windows Media Player, QuickTime, and uploads clean to YouTube.
  - DONE (browser path; Electron ffmpeg deferred). `engine/exporter.ts`: every output frame renders through the **same compositor as the preview** (`engine/compositor.ts`, extracted from the play/still paths) at export resolution, always from ORIGINAL media — per-clip sequential `CanvasSink` readers (decode-each-packet-once), never proxies. Audio: per-clip windows decoded and mixed in an `OfflineAudioContext` with volume/fades/`playbackRate` speed, then AAC. Presets: YouTube 4K (36 Mbps) / 1080p (8 Mbps) / social 9:16 (10 Mbps) / match-project; codec = first encodable of avc→vp9→av1 (checked per resolution); estimated size, progress chip, cancel. Export via top-bar Export, toolbar Export… dialog, storyboard Export, palette. Deferred: H.265/ProRes via bundled ffmpeg (belongs with P9.3 Electron packaging — no ffmpeg binary is shipped yet). AC not machine-verified here (needs the in-browser run — P1.8 proxy caveat); the realtime-encode path (hardware WebCodecs, canvas-source backpressure) and MP4 muxing are the standard mediabunny pipeline.
  - Ceiling notes: OfflineAudioContext mixing holds the whole timeline's audio in memory (fine to ~10 min; chunked mixing is the upgrade); export runs on the main thread (worker offload is a P9.5 candidate).
- [ ] P3.12 **Phase done when:** storyboard AND timeline flows both produce a correct 4K export; project survives close/reopen via autosave.
  - STATUS: all P3 feature tasks (P3.1–P3.11) implemented; 131 unit tests green; web + Electron builds clean. Autosave restore is implemented (P3.2). NOT ticked pending the in-browser end-to-end pass (assemble in both modes → 4K export plays correctly), blocked by the same P1.8 caveat: Claude-in-Chrome on this machine routes to a different Studio checkout, so the manual run needs a human (or a fixed proxy) on this build.
