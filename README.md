...
- [ ] P1.6 Insert (blank / from another PDF / from image), delete, duplicate, rotate, crop pages.
  - MOSTLY DONE: insert-blank / insert-from-PDF / insert-from-image / delete / duplicate / rotate all implemented as undoable commands and unit-tested; delete verified in-browser. **Crop pages: NOT done** — the `cropPages` mutation exists and is not yet wired to an interactive crop-box tool (deferred). Not ticking until crop lands.
- [ ] P1.7 Extract selected pages to new PDF; split by range / every N pages / by top-level bookmark / by target filesize.
  - PARTIAL: extract-selected (→ new doc) and split-every-N (→ downloaded files) done + unit-tested. **Split by range / by bookmark / by target filesize: NOT done** (range is a small add; bookmark needs outline parsing; filesize needs an iterative quality loop). Not ticking until those land.
- [ ] P1.8 Merge: drop multiple PDFs+images onto a merge tray, drag-to-order, combine. AC: merging preserves bookmarks and form fields of source documents.
  - PARTIAL: `pdf.merge` concatenates multiple PDFs (active doc first) into a new document, unit-tested. **Missing:** the drag-to-order tray UI (merges in file-picker order for now), image inputs in the tray, and — critically — the AC: pdf-lib `copyPages` does **not** preserve interactive AcroForm fields or outlines/bookmarks across a merge. Meeting the AC needs manual AcroForm + outline merging. Not ticking until the AC passes.

Annotation
- [ ] P1.9 Text markup from real text selection: highlight, underline, strikethrough, squiggly — stored as proper PDF annotations (pdf-lib), not overlay graphics.
- [ ] P1.10 Sticky notes with replies + resolve state; freehand ink; shapes (rect, ellipse, line, arrow, polygon); text boxes/callouts. Colour/width pickers using theme palette.
- [ ] P1.11 Stamps: preset library (Approved, Draft, Confidential, Final) + custom image stamps saved locally.
- [ ] P1.12 Measurement tools for plans/drawings: distance, perimeter, area — with scale calibration ("this line = 10 m") persisted per document.
- [ ] P1.13 Comment list panel: filter by author/type/page, click → jump, export summary as text.

Forms & signatures (fill tier)
- [ ] P1.14 Fill AcroForms (text, checkbox, radio, dropdown, listbox, date); tab-order navigation; flatten command.
- [ ] P1.15 Signature library: draw (pointer/touch), type (3 cursive font options), upload PNG; stored locally (IndexedDB); place/resize/date-stamp on page; flatten on save.
